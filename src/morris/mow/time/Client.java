...
public class Client extends Fragment implements OnPageSelectedListener {
	...
	//Use data acquired during initrializeData() to create the view
		...
		listView.setOnChildClickListener(new OnChildClickListener() {
			...
			public boolean onChildClick(ExpandableListView arg0, View arg1,
				...
				if (started == null) {
					...
					case 0:
						startTimer(groupPosition);
						break;
					case 1:
						editClient(groupPosition);
						break;
					case 2:
						callClient(groupPosition);
						break;
					case 3:
						viewHistory(groupPosition);
						break;
					case 4:
						removeClient(groupPosition);
					}
				} else {
					// A lawn is being cut and it isn't paused
					if (allClients.get(groupPosition).getRowID() == started
							.getRowID() && !started.isPaused()) {
						// different set of options
						switch (childPosition) {
						case 0:
							stopTimer(groupPosition);
							break;
						case 1:
							pauseClient(groupPosition);
							break;
						case 2:
							callClient(groupPosition);
							break;
					...
					} else if (allClients.get(groupPosition).getRowID() == started
							.getRowID() && started.isPaused()) {
						switch (childPosition) {
						case 0:
							unpauseClient(groupPosition);
							break;
						case 1:
							callClient(groupPosition);
							break;
	...
	// Get needed data from SQLite table
	public void initializeData() {
		// Get all clients from the data manager
		allClients = null;
		allClients = manager.getAllLawns();
		...
		if (started == null) {
			...
			for (int i = 0; i < allClients.size(); i++) {
				...
				cut_date = sdf.format(new Date(allClients.get(i).getToBeCut()));
			...
			for (int i = 0; i < allClients.size(); i++) {
				...
				if (allClients.get(i).getRowID() == started.getRowID()
						&& started.isPaused()) {
					String cut_date = "";
					SimpleDateFormat sdf = new SimpleDateFormat("MMM dd, yyyy");
					sdf.setTimeZone(TimeZone.getTimeZone("GMT"));
				...
				else if (allClients.get(i).getRowID() == started.getRowID()
						&& !started.isPaused()) {
					String cut_date = "";
					SimpleDateFormat sdf = new SimpleDateFormat("MMM dd, yyyy");
					sdf.setTimeZone(TimeZone.getTimeZone("GMT"));
	...
	private void editClient(int index) {
		...
		new_cal.setTimeInMillis(temp.getToBeCut());
		date.updateDate(new_cal.get(Calendar.YEAR), new_cal.get(Calendar.MONTH), new_cal.get(Calendar.DAY_OF_MONTH)+1);
		...
		add.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				try {
					...
					Toast.makeText(getActivity(),"oops 2",
							Toast.LENGTH_LONG).show();
				}
	...
	private void pauseClient(int groupPosition) {
		...
		long pause_time = manager.pause(allClients.get(groupPosition)
				.getRowID());
		SimpleDateFormat sdf = new SimpleDateFormat("hh:mm a");
		sdf.setTimeZone(TimeZone.getTimeZone("GMT"));
		Toast.makeText(getActivity(),
	...
	protected void unpauseClient(int groupPosition) {
		...
		long pauseLength = manager.unpause(allClients.get(groupPosition)
				.getRowID());
		int min = (int) ((pauseLength / (1000 * 60)) % 60);
		int hours = (int) ((pauseLength / (1000 * 60 * 60)) % 24);
		Toast.makeText(getActivity(),
				" Pause Time: " + hours + " hours " + min + " minutes",
				Toast.LENGTH_SHORT).show();
		initializeData();
		createView();
	}
